...
public class AddressSearchTextField: UITextField {
    ...
    override public func willMoveToSuperview(newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(AddressSearchTextField.textFieldDidChange), forControlEvents: .EditingChanged)
        self.addTarget(self, action: #selector(AddressSearchTextField.textFieldDidBeginEditing), forControlEvents: .EditingDidBegin)
        self.addTarget(self, action: #selector(AddressSearchTextField.textFieldDidEndEditing), forControlEvents: .EditingDidEnd)
        
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(AddressSearchTextField.keyboardWillShow(_:)), name: UIKeyboardWillShowNotification, object: nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(AddressSearchTextField.keyboardWillHide(_:)), name: UIKeyboardWillHideNotification, object: nil)
    }
    ...
    public func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = NSTimer.scheduledTimerWithTimeInterval(0.8, target: self, selector: #selector(AddressSearchTextField.typingDidStop), userInfo: self, repeats: false)
...
extension AddressSearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCellWithIdentifier(AddressSearchTextField.cellIdentifier)
        
        if cell == nil {
            cell = UITableViewCell(style: .Subtitle, reuseIdentifier: AddressSearchTextField.cellIdentifier)
        }
    ...
    public func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if itemSelectionHandler == nil {
            ...
            self.address = filteredResults[indexPath.row].address
        }
